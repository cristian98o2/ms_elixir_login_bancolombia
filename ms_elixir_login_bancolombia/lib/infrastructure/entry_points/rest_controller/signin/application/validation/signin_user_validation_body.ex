defmodule MsElixirLoginBancolombia.Infrastructure.EntryPoint.Signin.SigninUserValidationBody do
  @moduledoc """
  Module to validate the body for Signin
  """
  ...
  def validate_body(body) do
    with {:ok, true} <- validate_map(body),
         ...
         {:ok, true} <- validate_fields(body.data, :password) do
      {:ok, :true}
    else
      error ->
        error
